...
class Black_Scholes_Model:
    ...
    def generate_paths(self, alpha, integrand, seed, QMC_sample=False, GPCA = False):
        ...
        if GPCA == False:
            if QMC_sample:
                # print('Using QMC')
                # Initialize scrambled Sobol generator
                engine = Sobol(d=self.m, scramble=True)

                # Generate scrambled Sobol samples
                sobol_samples = engine.random_base2(alpha)

                # Convert Sobol samples (uniform distribution) to normal distribution using inverse transform sampling
                X = norm.ppf(sobol_samples)
            else:
                # print('Using MC')
                n = 2**alpha
                X = np.random.default_rng(seed=seed).normal(size=(n,self.m))
            ...
            if QMC_sample:
                XX = gPCA.get__GPCA_samples(integrand, dim = self.m, mean=None, cov=None, m=alpha, method='QMC', GPCA=True)
                paths = self.S0 * np.cumprod(np.exp((self.r - 0.5 * self.𝜎 ** 2) * dt +
                self.𝜎 * np.sqrt(dt) * XX), 1) 

            else:
                XX = gPCA.get__GPCA_samples(integrand, dim = self.m, mean=None, cov=None, m=alpha, method='MC', GPCA=True)
                paths = self.S0 * np.cumprod(np.exp((self.r - 0.5 * self.𝜎 ** 2) * dt +
                self.𝜎 * np.sqrt(dt) * XX), 1) 
        # print(paths)
        return paths

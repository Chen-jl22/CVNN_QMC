...
class Option:
    ...
    def MC_pricer(self, alpha, integrand, seed, QMC_sample,GPCA):
        ...
        paths = self.model.generate_paths(alpha=alpha, integrand=integrand,seed=seed, QMC_sample=QMC_sample,GPCA=GPCA)
        # compute the contracts (payoffs) 
        particles = self.contract(paths)
        # compute evaluataions phi(𝑋_1), ...,phi(𝑋_n) 
        eval_phi = np.clip(particles-self.K, a_min=0, a_max=None).ravel()
        # Monte Carlo estimate is average
        mc = np.mean(eval_phi)
        # print(eval_phi.shape)
        true = 6.035564
        err_mc = np.sqrt(np.mean((eval_phi-true)**2))
        # err_mc = np.std(a=eval_phi,ddof=1)
        return self.discount*mc, err_mc
        # return self.discount*mc
        
        
    def CVNN_pricer(self, alpha1, alpha2,integrand, seed, QMC_sample, GPCA):
        ...
        '''
        # generate prices paths with model (Black-Sholes or Merton)
        paths = self.model.generate_paths(alpha=alpha1, integrand=integrand, seed=seed, QMC_sample=QMC_sample,GPCA=GPCA)
        # compute the contracts (payoffs) 
        particles = self.contract(paths)
        # compute evalutaions phi(𝑋_1), ...,phi(𝑋_n) 
        eval_phi = np.clip(particles-self.K, 0, None).ravel()
        # Monte Carlo estimate is average
        # mc = np.mean(eval_phi)
        ...
        # generate prices paths with model (Black-Sholes or Merton)
        paths_mc = self.model.generate_paths(alpha=alpha2,integrand=integrand,seed=seed, QMC_sample=QMC_sample,GPCA=GPCA)
        ...
        true = 6.035564
        err_mcnn = np.sqrt(np.mean((eval_nn-true)**2))
        # err_mcnn = np.std(eval_nn,ddof=1)
        return self.discount*mcnn, err_mcnn
         
    def CV_pricer(self, alpha, integrand, seed, QMC_sample, GPCA): 
...
class European_call_option(Option):
    ...
    def integrand(self):
        dt = self.T/self.m
        g = lambda x: self.S0*np.exp((self.r - 0.5*(self.𝜎)**2)*self.T + self.𝜎*np.sqrt(dt)*np.sum(x))
        return g
